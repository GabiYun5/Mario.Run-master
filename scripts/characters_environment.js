...
function game() {
  ...
  if (gameConfig.status === 'start') {
    ...
    textSize(40);
    textAlign(CENTER);
    text("Presiona el boton de iniciar para comenzar ", gameConfig.screenX / 2, gameConfig.screenY / 2);
    textSize(40);
  ...
  if (gameConfig.status === 'gameover') {
    ...
    textSize(15);
    text("Presiona ESPACIO para rempesar", gameConfig.screenX / 2, gameConfig.screenY / 2 + 135);
    textSize(40);
...
function play() {
  if (gameConfig.status === "start" || gameConfig.status === "gameover") {
...
function manualControl(character) {
  ...
  if (character.live) {
    if (keyDown(control.left) || narizx > 222) {
      character.velocity.x -= gameConfig.moveSpeed;
      character.changeAnimation('move');
      character.mirrorX(-1);
    } else if (keyDown(control.right) || narizx < 222) {
      character.velocity.x += gameConfig.moveSpeed;
      character.changeAnimation('move');
      character.mirrorX(1);
    } else {
...
function jumping(character) {
  if (character.live) {
    if ((keyWentDown(control.up)) || (touchIsDown) || narizy < 75) {
